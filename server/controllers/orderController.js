...
export const placeNewOrder = catchAsyncErrors(async (req, res, next) => {
    ...
    // (for loop so `return next()` actually stops the function)
    // ================================
    ...
    const shipping_price = total_price >= 1000 ? 0 : 100;
    ...
    console.log("[placeNewOrder] New order created with id:", orderId);
    ...
    // ================================
    console.log("[placeNewOrder] Items to decrement stock for:", items);

    for (const item of items) {
        console.log(
            "[placeNewOrder] Decrementing stock -> product:",
            item.product.id,
            "qty:",
            item.quantity
        );
        ...
        console.log(
            "[placeNewOrder] Stock update result:",
            stockUpdateResult.rows[0]
        );
    }
...
export const fetchAllOrders = catchAsyncErrors(async (req, res, next) => {
    ...
    // GROUP BY o.id, s.id, u.id  -> so admin gets user's name/email with each order
...
export const updateOrderStatus = catchAsyncErrors(
    async (req, res, next) => {
        ...
        if (!status) {
            return next(
                new ErrorHandler(
                    "Provide a valid status for order.",
                    400
                )
            );
        }
        ...
        if (results.rows.length === 0) {
            return next(
                new ErrorHandler("Invalid order ID.", 404)
            );
        }
