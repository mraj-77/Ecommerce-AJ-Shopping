...
const LoginModal = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // User login hone per form reset
    if (authUser) {
      setFormData({ name: "", email: "", password: "", confirmPassword: "" });
    }
  };

  // Agar popup band hai ya user pahale se logged in hai, to kuchh bhi render n kare
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 glass-panel w-full max-w-md mx-4 animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* CONFIRM PASSWORD - ALWAYS VISIBLE EXCEPT FOR RESET` MODE */}
          {mode === "reset" && (
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-muted-foreground" />
              <input
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full py-3 gradient-primary flex justify-center items-center gap-2 text-primary-foreground rounded-lg font-semibold animate-smooth ${isLoading
